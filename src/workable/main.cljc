(ns workable.main
  "Kotodama WASM entrypoint for the Workable clean-room actor (L4) — Clojure port.
  ...
  facts (`workable.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "workable")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"workable.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "workable-compat" :tier tier :entities entities} 200])
...
(defn start! [] :workable-compat/ready)
